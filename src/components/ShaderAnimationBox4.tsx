...
import styles from "./ShaderAnimationBox4.module.css";
...
function ShaderCanvas({
  ...
  useEffect(() => {
    ...
    function tick(now: number) {
      if (disposed) return;
      if (!gl || gl.isContextLost()) { rafRef.current = requestAnimationFrame(tick); return; }
      ...
      try {
        ...
        if (gl) drawError(gl, (err as Error)?.message ?? String(err));
      }
  ...
  return (
    <div className={styles.shaderAnimationBox4}>
      <canvas ref={canvasRef} />
    </div>
  );
}

/* ========= Default export: container for box ========= */
export default function ShaderAnimationBox4() {
  return (
    <div className={styles.shaderAnimationBox4}>
